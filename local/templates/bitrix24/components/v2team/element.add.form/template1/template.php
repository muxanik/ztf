...
<?$ii = 0;?>
<style>
	.newt tbody tr:first-child {display:none;}
</style>
<form name="iblock_add" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data">
	<?=bitrix_sessid_post()?>
	<?if ($arParams["MAX_FILE_SIZE"] > 0):?><input type="hidden" name="MAX_FILE_SIZE" value="<?=$arParams["MAX_FILE_SIZE"]?>" /><?endif?>
	<table class="data-table newt">
		<thead>
					...
					<td>
						...
						<textarea cols="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]?>" rows="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["ROW_COUNT"]?>" name="PROPERTY[<?=$propertyID?>][<?=$i?>]"><?=$value?></textarea>
							...
							case "N":
								...
								?>
								<input id="inp<?=$ii++;?>" type="text" name="PROPERTY[<?=$propertyID?>][<?=$i?>]" size="25" value="<?=$value?>" /><br /><?
		...
		<tfoot>
			...
			<tr>
				<td colspan="2">
					<input type="submit" name="iblock_submit_z" value="<?=GetMessage("IBLOCK_FORM_SUBMIT")?>" />
					<?if (strlen($arParams["LIST_URL"]) > 0):?>
						<input type="submit" name="iblock_apply_z" value="<?=GetMessage("IBLOCK_FORM_APPLY")?>" />
						<input
							type="button"
							name="iblock_cancel"
...
inp1.addEventListener('keyup', joinValues, false);
inp2.addEventListener('keyup', joinValues, false);
inp3.addEventListener('keyup', joinValues, false);
inp4.addEventListener('keyup', joinValues, false);
inp5.addEventListener('keyup', joinValues, false);


function joinValues(){
    inp0.value = inp1.value + ' ' + inp2.value + ' ' + inp3.value + ' ' + inp4.value + ' ' + inp5.value;
}
